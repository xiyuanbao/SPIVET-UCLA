...
pdifname  = "SIDE-PLNRVEL.ex2"    # Input file for PIVData object.

velkey = "U-MF-SN-ZD-GS"            # Velocity key for vort, divrg, trace.
tssdiv = 10               # Time steps per Epoch for tracing.

# Passive tracer setup for mpsvtrace().
epslc    = slice(33,40)                  # Epoch slice for processing.
direc    = -1
eplim    = [30,39] 
ntrpc    = [1,4,4]                      # Number of tracers per cell per call.
ncalls   = 1                            # Number of calls. >1 not implemented in FTLE core lib.
irspath  = "SIDE-PLANE-FTLETRACE-BWD-NOBACKUP"     # Intermidate results output path.
pdofname = "SIDE-PLANE-FTLEFIELD-BWD.ex2"          # Output file for PIVData object.
