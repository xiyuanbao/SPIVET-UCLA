...
class tpswarp():
    ...
    """ 
    def __init__(self,tfpts,sfpts,lmda=0.,forwardonly=False):#Xiyuan add forward flag
        """
        ----
...
def drwcrc(imdim,params):
	...
	try:
		...
		print im.shape,imdim,i,params[i,0],params[i,1],params.shape
...
def linht(imin,rbndx,dtheta,drho,show=True):
    ...
    #Xiyuan
    #print dtmx,dtheta,ceil(dtmx/dtheta)
    rtheta = indices([int(2*ceil(dtmx/dtheta) +1)])
    ...
    #print rrhosz
    rrhosz = int(rrhosz)
    rrho   = indices([rrhosz]).reshape(rrhosz)
    ...
    #[cline,params] = pkfind(ntpad,dtheta,htmat[0:ntheta,:],pkbs,htthold,pkbs,False)
    ...
    myparams = []
    for param in params:
    	if abs(param[0])<0.03 or abs(param[0]-1.57)<0.1:
        	myparams.append(list(param))
    params = array(myparams)
...
#################################################################
#Xiyuan: restrict candidate theta to be near 0 and Pi
#def pkfind(ntpad,dtheta,data,p1bsize,p1thold,p2bsize,show=True):
def pkfind(data,p1bsize,p1thold,p2bsize,show=True):
    ...
    for m in range(nyblks):
        ...
        for n in range(nxblks):
            ...
            #Xiyuan
            #mytheta = (pbsndx[0]+xarg-ntpad)*dtheta
            #if mytheta<0.2 or abs(mytheta-1.57)<0.15:
