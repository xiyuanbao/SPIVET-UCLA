...
class dewarpimg(spivetstep):
    ...
    def execute(self):
        ...
        if ( self.m_config.has_key('padpix') ):
            ...
            padpix = 0
        #Xiyuan
        if ( self.m_config.has_key('intensity_planes') ):
            intensity_planes = self.m_config['intensity_planes']
        else:
            intensity_planes = None
        ...
        #Xiyuan: Intensity correction
        if intensity_planes is not None:
            cplane = simgprms[0][0]['S']
            if ( intensity_planes.__contains__(cplane) ):
                for cam in range(ncam):#modify only last channel(blue or intensity)                    
                    for frm in range( len(simgchnls[0]) ):
                        
                        crc = simgchnls[cam][frm][-1].copy()
                        new_img = crc.copy()
                        for i in range(1):
                        ...
                        medianimg_new = cv2.fastNlMeansDenoising(crc8,20,20,11,33)
                        medianimg_new = (medianimg_new/255.0)*(maxd-mind)+mind
                        medianimg_new[medianimg_new<1e-3]=1e-3
                        intensity_ref = medianimg_new
                    
                    
                        new_img = new_img/intensity_ref
                        new_img = new_img/np.median(new_img)*0.3
                        new_img[new_img>1] = 1
                        simgchnls[cam][frm][-1] = new_img
...
class oflow2d(spivetstep):
    ...
    def execute(self):
        ...
        # Xiyuan: if 'high_res' is True, 
        # change the pdname to 'lpivdata'
        # ->low resolution in oflow2d
        if ( pivdict.has_key('high_res') ):
        ...
        #Xiyuan: set default crop range as entire warped image
        if not ( pivdict.has_key('gp_rbndx') ):
            imdim=imgchnls[cam][0][2].shape
            pivdict['gp_rbndx'] = array([[0,imdim[0]],[0,imdim[1]]])
		...
		try:
			rslt = pivlib.ofcomp_gpu(imgchnls[cam][0][2],
                                 imgchnls[cam][1][2],
                                 pivdict)
		except:
...
#################################################################
#Xiyuan
class opflow2d(spivetstep):
    """
    STEP: per pixel 2D Optical flow using Calculus of variation
    ...
    def execute(self):
        ...
        # Setup the PIVEpoch and PIVData objects.
        # Create high resolution 'pivdata'
        if ( carriage.has_key('pivdata') ):#plane 0 exists
            pd = carriage['pivdata']
            pe = pd[0]
        else:
            pe  = pivlib.PIVEpoch(0)
...
#             origin = rbndx[:,0] -pado
            origin = [0.,origin[0],origin[1]]
            #cellsz and origin will be updated in oflow3d
            pd = pivlib.PIVData(cellsz,origin,"PIVDATA")
            pd.append(pe)
        ...
        # Compute two-component flow for each camera.
        # Use optical flow to refine displacement. It is a python wrapper of 
        # Ce Liu's C++ implementation of Coarse2Fine Optical Flow from
        # Brox 2004, without the gradient consistency but with additional 
        ...
        for cam in range(ncam):
            ...
            im1 = imgchnls[cam][0][2][rbndx[0,0]:rbndx[0,1],rbndx[1,0]:rbndx[1,1]]
            im2 = imgchnls[cam][1][2][rbndx[0,0]:rbndx[0,1],rbndx[1,0]:rbndx[1,1]]
            h,w=im1.shape
            ...
            ratio = 0.5
            minWidth = 16
            nOuterFPIterations = 5#more iters than the hybrid version!
            ...
            e = time.time()
            print('Optical Flow: %.2f seconds for image of size (%d, %d)' % (
                e - s, h, w))


            print "Current  RUN TIME: %f" % ( time.time() -a )

            posttime = time.time()
            #Hank's filter takes ~20s, too slow, use opencv filter instead
            #opencv is 10x faster than scipy, scipy is 2~4x than Hank's
            u = fast_filter(u)
            v = fast_filter(v)

            crc = pivlib.PIVVar([3,1,h,w],'U','PIX')
            crc[1,0,:,:] = v
            crc[2,0,:,:] = u

            print "Filter RUN TIME: %f" % ( time.time() -posttime )
            
            cmag = crc[1:3,...].reshape([2,-1])
            cmag = sqrt( (cmag*cmag).sum(0) )
            cmmx = cmag.max()
            print "OPYFLOW MAX |CRC|: %f" % (cmmx)
            
            a    = time.time() -a
            print "CAM %i OFCOMP RUN TIME: %g" % (cam,a)
            
            ofDisp = crc
            ofDisp.setAttr("R%i" % cam,'PIX')
...
class refine2dof(spivetstep):
    ...
    of thin plate splines. Only for selected region, suing CCPIV.
    Deal with 'pivdata' from oflow2d step.
	...
	#Xiyuan
        ...
        if ( runfltr ):
            ...
            fimgs = imgs
#         print "Current  RUN TIME3: %f" % ( time.time() -a )            
	...
	for c in xrange(ncam):
        ...
        if ( fltronly ):
            return            
        print "Current  RUN TIME5: %f" % ( time.time() -a )
        ...
        ocrbndx = crbndx
        print "Current  RUN TIME6: %f" % ( time.time() -a )
    ...
    #             import pickle
    #             with open("dbccrds", "wb") as fp:
    #                 pickle.dump(dbccrds, fp)
    #             with open("bccrds", "wb") as fp:
    #                 pickle.dump(bccrds, fp)
    #             with open("disp", "wb") as fp:
    #                 pickle.dump(var[1:3,0,...], fp)
    #             with open("xrrfn", "wb") as fp:
    #                 pickle.dump(xrrfn, fp)
                print "cam %d initiate tps:%.2f seconds" % (cam,time.time()-start1)
                ...
                print "cam %d finish tps:%.2f seconds" % (cam,time.time()-start2)
    ...
    #                             pickle.dump(xrrfn, fp)
                ...
                wimg = pivlib.pivutil.pxshift(fimgs[cam][0],
                                              xrrfn.astype(int),p)
                start3=time.time()
                print "tpswarp time:",start2-start1,start3-start2
		...
		start4=time.time()                
                ...
                #Xiyuan
		start5=time.time()
		print "aftertps:",start4-start3,start5-start4
...
#################################################################
#
class refine2dof_whole(spivetstep):
    """
    STEP: Refine 2D Optical flow
    
    Modified by Xiyuan. The whole image will be refined with optical
    flow using a modified Brox 2004 Method.
    
    Deal with 'lpivdata' from oflow2d.
    ...
    def __regionprms__(self,pivdict):# new
        """
        Computes region bounding indices and block center coords.
        """
        ...
        # cell numbers of CCPIV blocks
        [rsize,lbso,hbso,lnblks,hnblks] = pivutil.getblkprm(rbndx,bsize,
                                                        bolap,bsdiv)
        crsize   = hnblks
        tnrcells = crsize.prod()
        #Let the refined area to be rbndx 
        xrrbndx = rbndx 
        xrrsize = xrrbndx[:,1] -xrrbndx[:,0]

        # Setup image indices and block center coordinates (in pixels) for the
        # refined region.  Coordinates are relative to the refined region.
        # The origin is [0,0] because the images will be cropped accordingly
...
#         xrrfn = xrrfn +xrrbndx[:,0]
        ...
        #indices per block center
        bco    = bsize/(2.*bsdiv)
        bccrds = indices(crsize,dtype=float)
        for i in xrange(2):
            bccrds[i,...] = ebsize[i]*bccrds[i,...] +bco[i]
...
#         from scipy.ndimage import median_filter
        ...
        imgchnls = carriage['imgchnls']
        imgprms  = carriage['imgprms']
        # low resolution from oflow2d
        pd       = carriage['lpivdata']
...
#             fltronly = False
        ...
        # Extract image regions corresponding to rbndx.
        ncam  = len(imgchnls)
        imgs = []
        for c in xrange(ncam):
            imgs.append([])
            for f in xrange(2):
                img = imgchnls[c][f][2][rbndx[0,0]:rbndx[0,1],
                                        rbndx[1,0]:rbndx[1,1]]
                imgs[c].append(img)
...
#             return            
        ...
        # Create high resolution 'pivdata'
        if ( carriage.has_key('pivdata') ):#plane 0
            pd_new = carriage['pivdata']
            pe = pd_new[0]
        else:
            pe  = pivlib.PIVEpoch(0)
            padpix = imgprms[0][0]['padpix']
...
#             origin = rbndx[:,0] -pado
            ...
            pd_new = pivlib.PIVData(cellsz,origin,"PIVDATA")
            pd_new.append(pe)

            carriage['pivdata'] = pd_new
        
        
        
        print "-v-v-v-v-v- REFINE2DOF_WHOLE -v-v-v-v-v-"
...
#             crbndx = ocrbndx
            rprms  = self.__regionprms__(pivdict)

            crsize   = rprms['crsize']
            tnrcells = rprms['tnrcells']
            xrrbndx  = rprms['xrrbndx']#same as rbndx
            xrrsize  = rprms['xrrsize']
#             rrbndx   = rprms['rrbndx']
            xrrfn    = rprms['xrrfn']
            wxrrfn   = rprms['wxrrfn']
            ...
            # We will refine the entire image
            # The refinement is based on the warped image 
            # using interpolated oflow2d result
            # no iterations about further CCPIV anymore
            # we don't look cmax anymore 
            # since openpiv does not give that
            # instead we look at the misfit between
            # f1_warped and f2
            


            # Get oflow2d variable.
            rvar = pd[0][varnm[cam]].copy()# The name will be used later
            var  = rvar
...
#                             pickle.dump(xrrfn, fp)
            ...
            ofDisp = pivlib.PIVVar([3,1,h,w],'U','PIX')
            ofDisp[1,0,:,:] = p[:,0].reshape(h,w)
            ofDisp[2,0,:,:] = p[:,1].reshape(h,w)
            
            #ofDisp_hybrid = pivlib.PIVVar([3,1,h,w],'U','PIX')
            ofDisp_hybrid = ofDisp + crc
            
            
#             # Warp frame 0 and calculate the difference to frame 1
#             p_hybrid = p.copy()

#             p_hybrid[:,0] = ofDisp_hybrid[1,0,...].flatten()
#             p_hybrid[:,1] = ofDisp_hybrid[2,0,...].flatten()
#             wimg_hybrid = pivlib.pivutil.pxshift(fimgs[cam][0],
#                                           xrrfn.astype(int),p_hybrid)
        
#             wimg_hybrid = wimg_hybrid.reshape(xrrsize)
            
#             img_diff = wimg_hybrid - fimgs[cam][1]
            
# #             I0 = pivlib.PIVVar([1,1,h,w],'I','NA')
# #             I1 = pivlib.PIVVar([1,1,h,w],'I','NA')
#             Idiff = pivlib.PIVVar([1,1,h,w],'I','NA')
            
# #             I0[0,0] = fimgs[cam][0]
# #             I1[0,0] = fimgs[cam][1]
#             Idiff[0,0] = img_diff
            
            print "REST  RUN TIME: %f" % ( time.time() -posttime )
            
            ofDisp_hybrid.setAttr("%s-TR" % rvar.name,'PIX')
            ofDisp.setAttr("%s-TPS" % rvar.name,'PIX')
#             I0.setAttr("I0-C%i" % cam,"NA")
#             I1.setAttr("I1-C%i" % cam,"NA")
#             Idiff.setAttr("R%iINAC" % cam,"NA")
            
            
#             rslt = [ofDisp_hybrid, ofDisp, I0, I1,Idiff]
            rslt = [ofDisp_hybrid]
            pe.addVars(rslt)
        

        print "RUN TIME: %f" % ( time.time() -a )
        print "-^-^-^-^-^- REFINE2DOF_WHOLE -^-^-^-^-^-"
...
class oflow3d(spivetstep):
    ...
    *pivdata - PIVData object containing the 3D optical flow vectors and 
    ...
    def execute(self):
        ...
        if ( self.m_config.has_key('shadow') ):
            shadow = self.m_config['shadow']
        else:
            shadow = False
        ...
        if not high_res:
            ...
            if wicsp:
                cellsz = array([0,1,1])*wicsp[0]['mmpx']
                cellsz[0] = zcellsz
                origin = rbndx[:,0]*wicsp[0]['mmpx']+wicsp[0]['wos']
                origin = [0.,origin[0],origin[1]]
            else:
                cellsz = [zcellsz,1,1]
                origin = rbndx[:,0]
        ...
        for e in range( nepc ):
            ...
            if not high_res:
                ...
                if shadow:
                    ofdisp2d0 = shadow_filter(ofdisp2d0)
                    ofdisp2d1 = shadow_filter(ofdisp2d1)
...
#                 import pickle
#                 with open("/home/xbao/LAB_test/PLUME-3_51V-B25_2-11032008/ANALYSIS/ofdisp3d", "wb") as fp:
#                     pickle.dump(ofdisp3d,fp)
                if not high_res:
                    ...
                    if shadow:
                        ...
                        Vz = ofdisp3d[0,0,:,:]
                        #smooth Vz
                        Vz = median_NL_filter(Vz,1,(None,20,7,21))
                        #scale Vz to match Vx
                        X_threshold = np.max(np.abs(ofdisp3d[2,0,:,:]))
                        Vz = Vz/np.max(np.abs(Vz))*X_threshold
                        #use mean for Vx, Vy instead
                        ofdisp3d_=(ofdisp2d0+ofdisp2d1)*0.5*wicsp[0]['mmpx']
                        ofdisp3d[1:]=ofdisp3d_[1:]
                        ofdisp3d[0,0,:,:]=Vz
                    ...
                    # result can be noisy for high resolution case
                    for i in range(3):
                        ofdisp3d[i,0,:,:] = fast_filter(ofdisp3d[i,0,:,:],blurry = True)
...
#################################################################
#A faster filter, ~1.3 seconds for ~1000x1000 matrix
def fast_filter(u,blurry = False):
    """
    Apply a series of median/gaussian/high pass filter to a 2d numpy matrix. 
    """
    # remove bulk refelction band: use mean
    blur=cv2.medianBlur(np.float32(u),3)
    medianflow=u.copy()
    for i in range(5):
        for j in range(3):
    ...
    #         blur = cv2.GaussianBlur(blur,(5,5),0)
        diff=np.abs(blur-medianflow)
        threshold=5*(np.median(diff)+0.05)
        index=diff>threshold
        medianflow[index]=np.mean(blur)

    # detect edge of reflection band: high pass
    ...
    img_rst = cv2.filter2D(medianflow,-1,kernel)
    img_rst = np.abs(img_rst)
    edge_thre = max(4*np.median(img_rst),0.5)



    medianflow_new=medianflow.copy()
    edge_index=np.abs(img_rst)>edge_thre
    medianflow_new[edge_index]=np.mean(medianflow_new)


    # generic filter: median test

    blur=cv2.medianBlur(np.float32(medianflow_new),3)
    for i in range(5):
        for j in range(3):
            blur=cv2.medianBlur(blur,3)
            blur=cv2.medianBlur(blur,5)
            blur = cv2.GaussianBlur(blur,(5,5),0)
        diff=np.abs(blur-medianflow_new)
        threshold=4*(np.median(diff)+0.01)
        index=diff>threshold
        medianflow_new[index]=blur[index]
        
    if blurry:
        medianflow_new=np.float32(medianflow_new)
        for i in range(3):
            medianflow_new = cv2.medianBlur(medianflow_new,3)
        medianflow_new = cv2.medianBlur(medianflow_new,5)
        medianflow_new = cv2.GaussianBlur(medianflow_new,(3,3),0)
    else:
        medianflow_new = cv2.GaussianBlur(medianflow_new,(1,1),0)
    return medianflow_new
...
class loop_epoch(spivetstep):
    ...
    def execute(self):
        ...
        steps = self.m_config['steps']
	#Xiyuan
	if ( self.m_config.has_key('workpath') ):
            workpath = self.m_config['workpath']
        else:
            workpath = None
	if ( self.m_config.has_key('GPU') ):
            GPU = self.m_config['GPU']
        else:
            GPU = False
	if ( self.m_config.has_key('multiprocess') ):
            MP = self.m_config['multiprocess']
        else:
            MP = True
		...
		if (not MP):
			...
			#Spawn processes in python2 does not work for cuda either
			q = Queue()
			procs=[]
			for cnt,crg  in zip(range(len(grp)),grp):
			...
			curtpath = os.getcwd()+"/"
			cnt = 0
			stepsin = globals()[self.m_wrkrstepsvn]
			with open("stepsin", "wb") as fp:
                        	pickle.dump(stepsin, fp)
                        for crg in grp:
				with open("crg"+str(cnt), "wb") as fp:
    					pickle.dump(crg, fp)

				try:#We want to see outputs in real time
					pid = subprocess.Popen("python "+workpath+"epoch_worker.py "+curtpath+"crg"+str(cnt)+" "+curtpath+"stepsin "+str(cnt)+" "+self.m_accobpath, shell=True)
					#pid = subprocess.Popen("python "+workpath+"epoch_worker.py "+curtpath+"crg"+str(cnt)+" "+curtpath+"stepsin "+str(cnt)+" "+self.m_accobpath, shell=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE)
                                except KeyboardInterrupt:
        				# Need to handle Ctrl-C explicitly and die.
					pid.kill()
        				raise
			...
			for pid in q:
				...
				stdo.append(stdoutdata);stde.append(stderrdata)
				flags.append(int(stdoutdata[-2]))
			print "stdo:",stdo
			#print "stde:",stde
			
			self.m_assemble = all(flags) 
			'''
            if ( self.m_assemble ):
