...
def mpsvtrace(
    ...
    interp=['L','L'], ndpd=True, pdifname=None
    ):
    """
    ----
    ...
    pdifname         # Xiyuan:file path allows async mpsvtrace
    ----
    ...
    for i in range(ncalls):
        ...
        # Advect tracers.
        p=Process(target = mpsvtrace_internal, 
                  args=(irspath,pd,vkey,icomp,ntrpc,tssdiv,
                        src,adjid,hist,i,ctcsp,
                        tcfun,epslc,
                        interp,ndpd,pdifname))
        p.start()
        #p.join()
        procs.append(p)
...
#################################################################
#
def mpsvtrace_internal(irspath,pd,vkey,icomp,ntrpc,tssdiv,
                        src,adjid,hist,i,ctcsp,
                        tcfun,epslc,
                        interp,ndpd,pdifname):
    print " | ----- GROUP %i -----" % i
    ...
    pd_= pivlib.loadpivdata(pdifname)

    mptw = MPTWriter(irspath)
    mptw.setCall(i)
    mptw.setTS(0)
       # Advect tracers.
    trdata = psvtrace(pd_,vkey,icomp,ntrpc,tssdiv,src=src,
                      adjid=adjid,hist=hist,rsd=i,ctcsp=ctcsp,
                      tcfun=tcfun,epslc=epslc,writer=mptw.write,
                      interp=interp,ndpd=ndpd)
...
def ndvel(pd,vkey,epslc):
    ...
    for e in xrange(len(pd)):
        #Xiyuan:error here if the same pd used in parallel
